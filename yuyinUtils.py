    ...
    for (dirpath, dirnames, filenames) in os.walk(wav_path):
        for filename in filenames:
            ...
            if filename.endswith('.wav') or filename.endswith('.WAV'):
                ...
                with open(filename_path+'.trn','rb') as textf:
                    data = textf.read().decode('UTF-8')
                    data =data.replace('..','/Users/tengyu/Downloads/data_thchs30').strip()
                    # print('data',data)
                    with open(data,'rt',encoding='UTF-8') as label_file:
                        label = label_file.readline().strip()
                        # print(label)
                        labels.append(label)
                        new_wav_files.append(filename_path)
...
if __name__ == '__main__':
    ...
    get_wavs_lables('D:/data_thchs30/data_thchs30/train','D:/data_thchs30/data_thchs30/data')